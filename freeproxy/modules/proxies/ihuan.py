...
class IhuanProxiedSession(BaseProxiedSession):
    ...
    def refreshproxies(self):
        ...
        try:
            ...
            if resp.status_code != 200: session.headers.update({'Referer': 'https://ip.ihuan.me'}); resp = session.get('https://ip.ihuan.me/ti.html')
        ...
        for data_json in data_json_items[1:]:
            try: resp = session.post('https://ip.ihuan.me/tqdl.html', data=data_json); parsed_html = etree.HTML(resp.text); panel_body = parsed_html.xpath('//div[@class="col-md-10"]/div[@class="panel panel-default"]/div[@class="panel-body"]/text()')
            ...
            for proxy_str in panel_body:
                try: proxy_str: str = proxy_str.strip(); ip, port = proxy_str.split(':')
                except Exception: continue
                proxy_info = ProxyInfo(source=self.source, protocol=protocol, ip=ip, port=port, anonymity=anonymity, in_chinese_mainland=in_chinese_mainland, country_code=country_code)
