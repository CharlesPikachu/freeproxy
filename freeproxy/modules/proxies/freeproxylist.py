...
class FreeproxylistProxiedSession(BaseProxiedSession):
    ...
    def refreshproxies(self):
        ...
        for tr in proxy_table.xpath('.//tr')[1:]:
            try:
                protocol = "".join(tr.xpath("./td[5]/text()")).strip().lower()
                proxy_info = ProxyInfo(source=self.source, protocol=protocol, ip="".join(tr.xpath("./td[1]/text()")).strip(), port="".join(tr.xpath("./td[2]/text()")).strip(), anonymity="".join(tr.xpath("./td[6]/text()")).strip().split(' ')[0], country_code="".join(tr.xpath("./td[3]/text()")).strip(), in_chinese_mainland=("".join(tr.xpath("./td[3]/text()")).strip().lower() in ['cn']))
            except:
                continue
            self.candidate_proxies.append(proxy_info)
