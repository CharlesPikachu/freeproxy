...
class SpysoneProxiedSession(BaseProxiedSession):
    ...
    def _extractspysproxies(self, html: str):
        soup, obf_script = (lambda soup: (soup, next((s.string for s in soup.find_all('script') if s.string and "eval(function(p,r,o,x,y,s)" in s.string), None)))(BeautifulSoup(html, 'html.parser'))
        if not obf_script or not (m := re.search(r"return p\}\('([^']+)',(\d+),(\d+),'([^']+)'\.split\('([^']*)'\),0,\{\}\)\)", obf_script)): return []
        ...
        for row in soup.find_all('tr', onmouseover=True):
            if len((tds := row.find_all('td'))) < 6: continue
            if not (ip_font := tds[0].find('font', class_='spy14')) or not ip_font.contents: continue
            ip = str(ip_font.contents[0]).strip(); script = tds[0].find('script')
            if not script or not script.string: continue
            if not (pairs := re.findall(r'(\w+)\^(\w+)', script.string)): continue
            port_digits = [str((env[left] if left in env else int(left)) ^ (env[right] if right in env else int(right))) for left, right in pairs]
            port = int("".join(port_digits)); type_text = tds[1].get_text(" ", strip=True).upper()
            protocol = 'socks5' if 'SOCKS5' in type_text else 'socks4' if 'SOCKS4' in type_text or 'SOCKS ' in type_text else 'https' if 'HTTPS' in type_text or 'SSL' in type_text else 'http'
    ...
    def refreshproxies(self):
        # initialize
        self.candidate_proxies, session = [], requests.Session(); urls: list[str] = []
        headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36"}
        # obtain proxies
        try: (resp := session.get('https://spys.one/en/free-proxy-list/', headers=self.getrandomheaders(base_headers=headers))).raise_for_status(); soup = BeautifulSoup(resp.text, 'lxml'); rows = soup.find_all('tr', class_=re.compile(r'^spy1x{0,3}$'))
        ...
        for url in urls:
            try: (resp := session.get(url, headers=self.getrandomheaders(base_headers=headers))).raise_for_status(); results = self._extractspysproxies(resp.text)
            except Exception: continue
