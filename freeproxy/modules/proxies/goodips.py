'''
Function:
    Implementation of GoodIPSProxiedSession
Author:
    Zhenchao Jin
...
class GoodIPSProxiedSession(BaseProxiedSession):
    ...
    def refreshproxies(self):
        ...
        for page in range(1, self.max_pages+1):
            # --https
            try:
                (resp := session.get(f'http://api.goodips.com/?num=60&anonymity=%E9%AB%98%E5%8C%BF&protocol=HTTPS&format=json&page={page}', headers=self.getrandomheaders(headers_override=headers), timeout=60)).raise_for_status()
            ...
            try:
                (resp := session.get(f'http://api.goodips.com/?num=60&anonymity=%E6%99%AE%E5%8C%BF&protocol=HTTPS&format=json&page={page}', headers=self.getrandomheaders(headers_override=headers), timeout=60)).raise_for_status()
            ...
            try:
                (resp := session.get(f'http://api.goodips.com/?num=60&anonymity=%E9%80%8F%E6%98%8E&protocol=HTTPS&format=json&page={page}', headers=self.getrandomheaders(headers_override=headers), timeout=60)).raise_for_status()
            ...
            # --http
            try:
                (resp := session.get(f'http://api.goodips.com/?num=60&anonymity=%E9%AB%98%E5%8C%BF&protocol=HTTP&format=json&page={page}', headers=self.getrandomheaders(headers_override=headers), timeout=60)).raise_for_status()
            ...
            try:
                (resp := session.get(f'http://api.goodips.com/?num=60&anonymity=%E6%99%AE%E5%8C%BF&protocol=HTTP&format=json&page={page}', headers=self.getrandomheaders(headers_override=headers), timeout=60)).raise_for_status()
            ...
            try:
                (resp := session.get(f'http://api.goodips.com/?num=60&anonymity=%E9%80%8F%E6%98%8E&protocol=HTTP&format=json&page={page}', headers=self.getrandomheaders(headers_override=headers), timeout=60)).raise_for_status()
            ...
            # --socks4
            try:
                (resp := session.get(f'http://api.goodips.com/?num=60&anonymity=%E9%AB%98%E5%8C%BF&protocol=Socks4&format=json&page={page}', headers=self.getrandomheaders(headers_override=headers), timeout=60)).raise_for_status()
            ...
            try:
                (resp := session.get(f'http://api.goodips.com/?num=60&anonymity=%E6%99%AE%E5%8C%BF&protocol=Socks4&format=json&page={page}', headers=self.getrandomheaders(headers_override=headers), timeout=60)).raise_for_status()
            ...
            try:
                (resp := session.get(f'http://api.goodips.com/?num=60&anonymity=%E9%80%8F%E6%98%8E&protocol=Socks4&format=json&page={page}', headers=self.getrandomheaders(headers_override=headers), timeout=60)).raise_for_status()
            ...
            # --socks5
            try:
                (resp := session.get(f'http://api.goodips.com/?num=60&anonymity=%E9%AB%98%E5%8C%BF&protocol=Socks5&format=json&page={page}', headers=self.getrandomheaders(headers_override=headers), timeout=60)).raise_for_status()
            ...
            try:
                (resp := session.get(f'http://api.goodips.com/?num=60&anonymity=%E6%99%AE%E5%8C%BF&protocol=Socks5&format=json&page={page}', headers=self.getrandomheaders(headers_override=headers), timeout=60)).raise_for_status()
            ...
            try:
                (resp := session.get(f'http://api.goodips.com/?num=60&anonymity=%E9%80%8F%E6%98%8E&protocol=Socks5&format=json&page={page}', headers=self.getrandomheaders(headers_override=headers), timeout=60)).raise_for_status()
