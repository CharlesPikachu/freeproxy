...
class ProxydbProxiedSession(BaseProxiedSession):
    ...
    def refreshproxies(self):
        ...
        headers = {
            ...
            "Accept-Language": "zh-CN,zh;q=0.9,en-US;q=0.8,en;q=0.7", "Cache-Control": "max-age=0", "Priority": "u=0, i", "sec-ch-ua": '"Chromium";v="142", "Google Chrome";v="142", "Not_A Brand";v="99"', 
        ...
        for page in range(1, self.max_pages+1):
            try:
                (resp := session.get(f'https://proxydb.net/?offset={(page - 1) * 30}', headers=self.getrandomheaders(headers_override=headers))).raise_for_status()
                soup = BeautifulSoup(resp.text, 'lxml'); soup = soup.select_one("table.table.table-sm.table-hover.table-striped"); trs = soup.select("tbody > tr")
            except:
                continue
            for item in trs:
                try:
                    ...
                    proxy_info = ProxyInfo(source=self.source, protocol=tds[2].get_text(strip=True).lower(), ip=tds[0].get_text(strip=True).strip(), port=port, anonymity=anonymity, country_code=country_code, in_chinese_mainland=(country_code.lower() in ['cn']), delay=int(float(re.match(r"^(\d*\.?\d+)", tds[6].get_text(strip=True)).group(1)) * 1000))
                except:
                    continue
