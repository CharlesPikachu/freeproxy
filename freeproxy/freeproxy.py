...
class ProxiedSessionClient():
    def __init__(self, proxy_sources: list = 'KuaidailiProxiedSession', init_proxied_session_cfg: dict = None, disable_print: bool = False, max_tries: int = 20):
        # logger handle
        self.logger_handle = LoggerHandle()
        # proxied sessions
        self.proxied_sessions: Dict[str, BaseProxiedSession] = dict(); proxy_sources = [proxy_sources] if isinstance(proxy_sources, str) else proxy_sources
        ...
        for source in (proxy_sources := [s for s in list(set(proxy_sources)) if s in ProxiedSessionBuilder.REGISTERED_MODULES]):
            self.logger_handle.info(f'{self.__class__.__name__}.__init__ >>> initializing {source}.', disable_print=disable_print)
            (module_cfg := copy.deepcopy(init_proxied_session_cfg))['type'] = source
            self.proxied_sessions[source], candidate_proxies = BuildProxiedSession(module_cfg=module_cfg), []
            try: candidate_proxies = self.proxied_sessions[source].refreshproxies()
            except Exception as err: self.logger_handle.error(f'{self.__class__.__name__}.__init__ >>> {source} (Error: {err})', disable_print=disable_print)
            if not candidate_proxies: self.proxied_sessions.pop(source, None); continue
    ...
    def get(self, url, **kwargs):
        for _ in range(self.max_tries):
            proxied_session: BaseProxiedSession = self.getrandomproxiedsession()[1]; proxied_session.setrandomproxy()
            try:
                ...
                kwargs['proxies'] = proxied_session.proxies; (resp := proxied_session.get(url, **kwargs)).raise_for_status()
                return resp
    ...
    def post(self, url, **kwargs):
        for _ in range(self.max_tries):
            ...
            try:
                ...
                kwargs['proxies'] = proxied_session.proxies; (resp := proxied_session.post(url, **kwargs)).raise_for_status()
                return resp
            except Exception as err:
    ...
    def getrandomproxy(self, proxy_format: str = 'requests'):
        for _ in range(self.max_tries):
            try: return self.getrandomproxiedsession()[1].getrandomproxy(proxy_format=proxy_format)
            except Exception: continue
    '''savetojson'''
    def savetojson(self, save_path: dict = './free_proxies.json'):
        touchdir(os.path.dirname(save_path)); free_proxies = {}
